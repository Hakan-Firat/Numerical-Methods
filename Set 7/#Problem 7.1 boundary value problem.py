#Problem 7.1 boundary value problem
...
def shooting_root(v, g, x1, x2, u1, u2, iterates):
    ...
    result = u_vals[-1] - u2  
    print(f"v: {v}, shooting_root: {result}")  # Debugging 
    return u_vals[-1] - u2  
...
x1, x2 = 1, 3
u1, u2 = 2, -1
v_guess = -1  # Initial guess for u'(x1)

x_vals, u_vals, iterates = solve_by_shooting(example_g, x1, x2, u1, u2, v_guess)
...
x1, x2 = 1, 3
u1, u2 = 2, -1
v_guess = -3  # Approximate Expected value

x_vals, u_vals, iterates, v_final = solve_by_shooting(linear_g, x1, x2, u1, u2, v_guess)

# Print the final estimated solution
target_linear_v = -3.4950
target_nonlinear_v = -2.0161
print(f"Computed initial derivative v'(1) for linear BVP: {v_final} (Target: {target_linear_v})")

# Create animation
fig, ax = plt.subplots(figsize=(8, 6))
ax.set_xlabel('x')
ax.set_ylabel('u(x)')
ax.set_title('Shooting Method Iterations (Linear)')
line, = ax.plot([], [], lw=2)
text = ax.text(0.05, 0.9, '', transform=ax.transAxes, fontsize=12, color='red')
ax.set_xlim(x1, x2)
ax.set_ylim(min(min(u) for _, u in iterates) - 0.5, max(max(u) for _, u in iterates) + 0.5)
...
plt.ylabel('u(x)')
plt.legend()
plt.title('Shooting Method Iterations with the tips')
plt.show()

# from the animation we can see that with the implementation of better initial value, we managed to converge after 5 iterations comapred to 9 iterations.
